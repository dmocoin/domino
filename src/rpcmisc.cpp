...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total domino balance of the wallet\n"
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized domino balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in domino/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in domino/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"dominoaddress\"\n"
            "\nReturn information about the given domino address.\n"
            "\nArguments:\n"
            "1. \"dominoaddress\"     (string, required) The domino address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"dominoaddress\", (string) The domino address validated\n"
...
CScript _createmultisig_redeemScript(const Array& params)
{
    ...
    if ((int)keys.size() < nRequired)
        throw runtime_error(
            sDMOrintf("not enough keys supplied "
...
#ifdef ENABLE_WALLET
        // Case 1: Domino address and we have full public key:
        CBitcoinAddress address(ks);
        if (pwalletMain && address.IsValid()) {
            CKeyID keyID;
            if (!address.GetKeyID(keyID))
                throw runtime_error(
                    sDMOrintf("%s does not refer to a key", ks));
            CPubKey vchPubKey;
            if (!pwalletMain->GetPubKey(keyID, vchPubKey))
                throw runtime_error(
                    sDMOrintf("no full public key for address %s", ks));
    ...
    if (result.size() > MAX_SCRIPT_ELEMENT_SIZE)
        throw runtime_error(
            sDMOrintf("redeemScript exceeds size limit: %d > %d", result.size(), MAX_SCRIPT_ELEMENT_SIZE));
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are domino addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) domino address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"dominoaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"dominoaddress\"  (string, required) The domino address to use for the signature.\n"
