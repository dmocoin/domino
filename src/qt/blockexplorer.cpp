...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::DMO, nValue);
    ...
    std::string BlockContentCells[] =
        {
            ...
            _("Difficulty"), sDMOrintf("%.4f", GetDifficulty(pBlock)),
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (domino.conf).");
            QMessageBox::warning(this, "Domino Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
