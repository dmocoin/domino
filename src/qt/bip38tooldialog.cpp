...
bool isValidPassphrase(QString sDMOassphrase, QString& strInvalid)
{
    for (int i = 0; i < sDMOassphrase.size(); i++) {
        if (!validChar.contains(sDMOassphrase[i], Qt::CaseSensitive)) {
            if (QString("\"'").contains(sDMOassphrase[i]))
                continue;

            strInvalid = sDMOassphrase[i];
            return false;
        }
    }
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    QString qsDMOassphrase = ui->passphraseIn_ENC->text();
    QString strInvalid;
    if (!isValidPassphrase(qsDMOassphrase, strInvalid)) {
    ...
    std::string encryptedKey = BIP38_Encrypt(addr.ToString(), qsDMOassphrase.toStdString(), key.GetPrivKey_256());
...
{
    string sDMOassphrase = ui->passphraseIn_DEC->text().toStdString();
