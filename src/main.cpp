...
#if defined(NDEBUG)
#error "Domino cannot be compiled without assertions."
#endif
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if (nHeight < 1000) {
        ret = 0;
    }  else if (nHeight <= 33000 && nHeight > 1000) {
        ret = blockValue * 0.8;
    } else if (nHeight <= 66000 && nHeight > 33000) {
        ret = blockValue * 0.5;
    } else if (nHeight <= 100000 && nHeight > 66000) {
        ret = blockValue * 0.3;
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        if (check())
                            return state.Invalid(false, REJECT_NONSTANDARD, sDMOrintf("non-mandatory-script-verify-flag (%s)", ScriptErrorString(check.GetScriptError())));
                    }
                    ...
                    return state.DoS(100, false, REJECT_INVALID, sDMOrintf("mandatory-script-verify-flag-failed (%s)", ScriptErrorString(check.GetScriptError())));
                }
            }
        }
    }
...
{
    return GetDataDir() / "blocks" / sDMOrintf("%s%05u.dat", prefix, pos.nFile);
}
...
bool static LoadBlockIndexDB()
{
    ...
    if (!fLastShutdownWasPrepared && !GetBoolArg("-forcestart", false) && !GetBoolArg("-reindex", false) && (vSortedByHeight.size() != vinfoBlockFile[nLastBlockFile].nHeightLast + 1) && (vinfoBlockFile[nLastBlockFile].nHeightLast != 0)) {
        ...
        if (!ReadBlockFromDisk(lastMetaBlock, pindexLastMeta)) {
            isFixed = false;
            strError = sDMOrintf("failed to read block %d from disk", pindexLastMeta->nHeight);
        }
        ...
        if (!ReadBlockFromDisk(lastBlock, pindexLast)) {
            isFixed = false;
            strError = sDMOrintf("failed to read block %d from disk", pindexLast->nHeight);
        }
    ...
    if (strCommand == "version") {
        ...
        if (pfrom->nVersion < ActiveProtocol()) {
            ...
            pfrom->PushMessage("reject", strCommand, REJECT_OBSOLETE,
                sDMOrintf("Version must be %d or greater", ActiveProtocol()));
...
{
    return sDMOrintf("CBlockFileInfo(blocks=%u, size=%u, heights=%u...%u, time=%s...%s)", nBlocks, nSize, nHeightFirst, nHeightLast, DateTimeStrFormat("%Y-%m-%d", nTimeFirst), DateTimeStrFormat("%Y-%m-%d", nTimeLast));
}
