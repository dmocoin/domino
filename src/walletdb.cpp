...
bool CWalletDB::WritePurpose(const string& strAddress, const string& sDMOurpose)
{
    nWalletDBUpdated++;
    return Write(make_pair(string("purpose"), strAddress), sDMOurpose);
}

bool CWalletDB::ErasePurpose(const string& sDMOurpose)
{
    nWalletDBUpdated++;
    return Erase(make_pair(string("purpose"), sDMOurpose));
}
        ...
        if (strType == "name") {
            ...
            if (31404 <= wtx.fTimeReceivedIsTxTime && wtx.fTimeReceivedIsTxTime <= 31703) {
                if (!ssValue.empty()) {
                    ...
                    strErr = sDMOrintf("LoadWallet() upgrading tx ver=%d %d '%s' %s",
                        wtx.fTimeReceivedIsTxTime, fTmp, wtx.strFromAccount, hash.ToString());
                    wtx.fTimeReceivedIsTxTime = fTmp;
                } else {
                    strErr = sDMOrintf("LoadWallet() repairing tx ver=%d %s", wtx.fTimeReceivedIsTxTime, hash.ToString());
            ...
            if (pwallet->mapMasterKeys.count(nID) != 0) {
                strErr = sDMOrintf("Error reading wallet database: duplicate CMasterKey id %u", nID);
                return false;
            }
...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // found.
    int64_t now = GetTime();
    std::string newFilename = sDMOrintf("wallet.%d.bak", now);
