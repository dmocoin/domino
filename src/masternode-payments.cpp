...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    BOOST_FOREACH (CMasternodePayee& payee, vecPayments) {
        ...
        if (payee.nVotes >= MNPAYMENTS_SIGNATURES_REQUIRED) {
            ...
            if (sDMOayeesPossible == "") {
                sDMOayeesPossible += address2.ToString();
            } else {
                sDMOayeesPossible += "," + address2.ToString();
            }
        }
...
bool CMasternodePaymentWinner::IsValid(CNode* pnode, std::string& strError)
{
    ...
    if (!pmn) {
        strError = sDMOrintf("Unknown Masternode %s", vinMasternode.prevout.ToStringShort());
    ...
    if (pmn->protocolVersion < ActiveProtocol()) {
        strError = sDMOrintf("Masternode protocol too old %d - req %d", pmn->protocolVersion, ActiveProtocol());
        LogPrintf("CMasternodePaymentWinner::IsValid - %s\n", strError);
        return false;
    }
    ...
    if (n > MNPAYMENTS_SIGNATURES_TOTAL) {
        ...
        if (n > MNPAYMENTS_SIGNATURES_TOTAL * 2) {
            strError = sDMOrintf("Masternode not in the top %d (%d)", MNPAYMENTS_SIGNATURES_TOTAL, n);
            LogPrintf("CMasternodePaymentWinner::IsValid - %s\n", strError);
