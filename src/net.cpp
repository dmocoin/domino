...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Domino users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    std::string port = sDMOrintf("%u", GetListenPort());
    ...
    if (r == 1) {
        ...
        string strDesc = "Domino " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (!addrBind.GetSockAddr((struct sockaddr*)&sockaddr, &len)) {
        strError = sDMOrintf("Error: Bind address family for %s not supported", addrBind.ToString());
        LogPrintf("%s\n", strError);
        return false;
    }
    ...
    if (hListenSocket == INVALID_SOCKET) {
        strError = sDMOrintf("Error: Couldn't open socket for incoming connections (socket returned error %s)", NetworkErrorString(WSAGetLastError()));
        LogPrintf("%s\n", strError);
        return false;
    }
    ...
    if (!SetSocketNonBlocking(hListenSocket, true)) {
        strError = sDMOrintf("BindListenPort: Setting listening socket to non-blocking failed, error %s\n", NetworkErrorString(WSAGetLastError()));
        LogPrintf("%s\n", strError);
        return false;
    }
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = sDMOrintf(_("Unable to bind to %s on this computer. Domino Core is probably already running."), addrBind.ToString());
        else
            strError = sDMOrintf(_("Unable to bind to %s on this computer (bind returned error %s)"), addrBind.ToString(), NetworkErrorString(nErr));
        LogPrintf("%s\n", strError);
    ...
    if (listen(hListenSocket, SOMAXCONN) == SOCKET_ERROR) {
        strError = sDMOrintf(_("Error: Listening for incoming connections failed (listen returned error %s)"), NetworkErrorString(WSAGetLastError()));
        LogPrintf("%s\n", strError);
        CloseSocket(hListenSocket);
        return false;
    }
...
bool CAddrDB::Write(const CAddrMan& addr)
{
    ...
    std::string tmpfn = sDMOrintf("peers.dat.%04x", randv);
