...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a Domino initiative\n"
            "  vote-alias         - Vote on a Domino initiative\n"
            "  vote               - Vote on a Domino initiative/budget\n"
    ...
    if (strCommand == "prepare") {
        ...
        if (params.size() != 7)
            throw runtime_error("Correct usage is 'mnbudget prepare proposal-name url payment_count block_start domino_address monthly_payment_domino'");

        std::string sDMOroposalName = params[1].get_str();
        if (sDMOroposalName.size() > 20)
        ...
        if (nBlockStart % GetBudgetPaymentCycleBlocks() != 0) {
            int nNext = pindexPrev->nHeight - pindexPrev->nHeight % GetBudgetPaymentCycleBlocks() + GetBudgetPaymentCycleBlocks();
            return sDMOrintf("Invalid block start - must be a budget cycle block. Next valid block: %d", nNext);
        }
        ...
        CBudgetProposalBroadcast budgetProposalBroadcast(sDMOroposalName, strURL, nPaymentCount, scriptPubKey, nAmount, nBlockStart, 0);
    ...
    if (strCommand == "submit") {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'mnbudget submit proposal-name url payment_count block_start domino_address monthly_payment_domino fee_tx'");
        ...
        CBudgetProposalBroadcast budgetProposalBroadcast(sDMOroposalName, strURL, nPaymentCount, scriptPubKey, nAmount, nBlockStart, hash);
    ...
    if (strCommand == "vote-many") {
        ...
        Object returnObj;
        returnObj.push_back(Pair("overall", sDMOrintf("Voted successfully %d time(s) and failed %d time(s).", success, failed)));
    ...
    if (strCommand == "getinfo") {
        ...
        CBudgetProposal* pbudgetProposal = budget.FindProposal(sDMOroposalName);
