...
public:
    ...
    void NewBlock();
    CBudgetProposal* FindProposal(const std::string& sDMOroposalName);
    CBudgetProposal* FindProposal(uint256 nHash);
    CFinalizedBudget* FindFinalizedBudget(uint256 nHash);
    std::pair<std::string, std::string> GetVotes(std::string sDMOroposalName);
...
public:
    ...
    void AutoCheck();
    //total domino paid out by this budget
...
public:
    ...
    */
    ...
    CBudgetProposal(std::string sDMOroposalNameIn, std::string strURLIn, int nBlockStartIn, int nBlockEndIn, CScript addressIn, CAmount nAmountIn, uint256 nFeeTXHashIn);
    ...
    std::string GetName() { return sDMOroposalName; }
    ...
    {
        //for syncing with other clients
        READWRITE(LIMITED_STRING(sDMOroposalName, 20));
...
public:
    ...
    CBudgetProposalBroadcast(std::string sDMOroposalNameIn, std::string strURLIn, int nPaymentCount, CScript addressIn, CAmount nAmountIn, int nBlockStartIn, uint256 nFeeTXHashIn);
    ...
    void swap(CBudgetProposalBroadcast& first, CBudgetProposalBroadcast& second) // nothrow
    {
        ...
        // by swapping the members of two classes,
        // the two classes are effectively swapped
        swap(first.sDMOroposalName, second.sDMOroposalName);
        swap(first.nBlockStart, second.nBlockStart);
